...
public class SocketApi {
    ...
    private boolean connector(IP Host,InetAddress LocalAddr,int LocalPort) throws IOException{
        Connections.put(Host.GetIP(),new Socket(Host.Converter(),Host.GetPort(),LocalAddr,LocalPort));
        log(" Connection Init to special client Successful");
        return true;
    }
    ...
    public boolean connection(IP Host,InetAddress LocalAddr,int LocalPort){
        try{
            connector(Host,LocalAddr,LocalPort);
        }catch(Exception E){
            return autoConnect(Host,LocalAddr,LocalPort);
        }
        return true;
    }
    ...
    public boolean autoConnect(IP Host,InetAddress LocalAddr,int LocalPort){
        int Tries = 0;
        boolean Connected = false;
        while (!Connected){
            try{
