...
public class Client{
    ...
    static SocketApi API = new SocketApi(false);
    static Crypt EnDe;
    static String Key = "";
    static UI MainUI;
    ...
    private void connectionProtocolAssist(String K){
        ...
        switch(K){
            ...
            default:
                String[] DeMessage;
                if (Key.equals("")){
                    DeMessage = K.substring(2).split(":");
                }else{
                    DeMessage = EnDe.Decrypt(K.substring(2), Key).split(":");
                }
                
                
                if (DeMessage[0].equals("Key")){
                    Key = DeMessage[1];
                    enSend("Main","Password:" + Serverpass);
                    System.out.println("Sent");
                }else if (DeMessage[0].equals("GoodPass")){
                    enSend("Main","@Start");
                }else if (DeMessage[0].equals("BadPass")){
                    NewLogin.dispose();
